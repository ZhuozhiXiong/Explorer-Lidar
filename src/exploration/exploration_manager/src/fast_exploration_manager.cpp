...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  nh.param("exploration/pd", ViewNode::pd_, -1.0);
  ...
  nh.param<string>("exploration/save_dir", save_dir, "");
  nh.param<string>("exploration/file_name", file_name, "traj_analysis_data.csv");
  ...
  time_log_file_ = save_dir + "/" + file_name;
  
  std::ofstream log_file;
  log_file.open(time_log_file_, std::ios::out | std::ios::trunc);

  if (log_file.is_open()) {
    log_file << "time,frontier,tsp,refine,traj,total\n";
    log_file.close();
    log_file_initialized_ = true;
    ROS_INFO_STREAM("\033[1;32m[Timer] Real-time log file created at: " << time_log_file_ << "\033[0m");
  } else {
      ROS_ERROR_STREAM("[Timer] Failed to create log file: " << time_log_file_);
  }
...
int FastExplorationManager::planExploreMotion(
    const Vector3d& pos, const Vector3d& vel, const Vector3d& acc, const Vector3d& pitch, const Vector3d& yaw) {
  
  if (!is_logging_initialized_) {
    start_time_ = ros::Time::now();
    is_logging_initialized_ = true;
  }
  ...
  frontier_finder_->computeFrontiersToVisit(pos,vel);
  frontier_finder_->getFrontiers(ed_->frontiers_);
  frontier_finder_->getFrontierBoxes(ed_->frontier_boxes_);
  frontier_finder_->getDormantFrontiers(ed_->dead_frontiers_);
  frontier_finder_->getFrontierNormal(ed_->pts_normals_, ed_->pts_positions_, ed_->pts_views_);

  if (ed_->frontiers_.empty()) {
    ROS_WARN("No coverable frontier.");
    return NO_FRONTIER;
  }
  frontier_finder_->getTopViewpointsInfo(pos, ed_->points_, ed_->pitches_, ed_->yaws_, ed_->averages_);
  for (int i = 0; i < ed_->points_.size(); ++i)
    ed_->views_.push_back(
        ed_->points_[i] + 2.0 * Vector3d(cos(ed_->pitches_[i])*cos(ed_->yaws_[i]), cos(ed_->pitches_[i])*sin(ed_->yaws_[i]), sin(ed_->pitches_[i])));

  double t_frontier = (ros::Time::now() - t1).toSec() * 1000.0;
  t1 = ros::Time::now();
  ...
  if (ed_->points_.size() > 1) {
    ...
    vector<int> indices;
    findGlobalTour(pos, vel, pitch, yaw, indices);

    t_tsp = (ros::Time::now() - t1).toSec() * 1000.0;
    t1 = ros::Time::now();
    ...
    if (ep_->refine_local_) {
      ...
      vector<vector<double>> n_pitches;
      frontier_finder_->getViewpointsInfo(
          pos, ed_->refined_ids_, ep_->top_view_num_, ep_->max_decay_, ed_->n_points_, n_pitches, n_yaws);

      ed_->refined_points_.clear();
      ed_->refined_views_.clear();
      vector<double> refined_yaws;
      vector<double> refined_pitches;
      refineLocalTour(pos, vel, pitch, yaw, ed_->n_points_, n_pitches, n_yaws, 
        ed_->refined_points_, refined_pitches, refined_yaws);
      next_pos = ed_->refined_points_[0];
      next_pitch = refined_pitches[0];
      next_yaw = refined_yaws[0];

      // Get marker for view visualization
      for (int i = 0; i < ed_->refined_points_.size(); ++i) {
        Vector3d view =
            ed_->refined_points_[i] + 2.0 * Vector3d(cos(refined_pitches[i])*cos(refined_yaws[i]), 
                    cos(refined_pitches[i])*sin(refined_yaws[i]), sin(refined_pitches[i]));
      ...
      for (int i = 0; i < ed_->refined_points_.size(); ++i) {
        ...
        frontier_finder_->percep_utils_->setPose_PY(ed_->refined_points_[i], refined_pitches[i], refined_yaws[i]);
        frontier_finder_->percep_utils_->getFOV_PY(v1, v2);
    ...
    if (ep_->refine_local_) {
      ...
      vector<vector<double>> n_pitches;
      frontier_finder_->getViewpointsInfo(
          pos, { 0 }, ep_->top_view_num_, ep_->max_decay_, ed_->n_points_, n_pitches, n_yaws);
      ...
      for (int i = 0; i < ed_->n_points_[0].size(); ++i) {
        auto tmp_cost = ViewNode::computeCost(
            pos, ed_->n_points_[0][i], pitch[0], n_pitches[0][i], yaw[0], n_yaws[0][i], vel, pitch[1], yaw[1], tmp_path);
      ...
      next_pitch = n_pitches[0][min_cost_id];
  ...
  // Compute time lower bound of yaw and pitch and use in trajectory generation
  double diff = fabs(next_yaw - yaw[0]);
  double time_lb_y = min(diff, 2 * M_PI - diff) / ViewNode::yd_;
  diff = fabs(next_pitch - pitch[0]);
  double time_lb_p = diff / ViewNode::pd_;
  double time_lb = max(time_lb_y, time_lb_p);
  ...
  if (planner_manager_->local_data_.position_traj_.getTimeSum() < time_lb_y - 0.1)
    ROS_ERROR("Lower bound not satified!");

  planner_manager_->planYawExplore(yaw, next_yaw, false, ep_->relax_time_);
  planner_manager_->planPitchExplore(pitch, next_pitch, true, ep_->relax_time_);

  ed_->next_pitch_ = next_pitch;
  ed_->next_yaw_ = next_yaw;

  double t_traj = (ros::Time::now() - t1).toSec() * 1000.0;
  double t_total = (ros::Time::now() - t2).toSec() * 1000.0;
  
  ROS_ERROR_COND(t_total > 100.0, "Total time too long!!!");

  if (log_file_initialized_) {
    std::ofstream log_file(time_log_file_, std::ios::out | std::ios::app);
    if (log_file.is_open()) {
  ...
  ROS_WARN("[Time] Frontier: %.2f ms, TSP: %.2f ms, Refine: %.2f ms, Traj: %.2f ms, Total: %.2f ms",
      t_frontier,  t_tsp, t_refine, t_traj, t_total);
...
void FastExplorationManager::findGlobalTour(
  ...
  frontier_finder_->updateFrontierCostMatrix();
  double frontier_mat_time = (ros::Time::now() - t1).toSec();
  t1 = ros::Time::now();

  frontier_finder_->getFullCostMatrix(cur_pos, cur_vel, cur_pitch, cur_yaw, cost_mat);
  const int dimension = cost_mat.rows();

  double full_mat_time = (ros::Time::now() - t1).toSec();
  t1 = ros::Time::now();
  ...
  ROS_WARN("Frontier mat: %lf, Full mat: %lf, TSP: %lf", frontier_mat_time, full_mat_time, tsp_time);
}

void FastExplorationManager::refineLocalTour(
    const Vector3d& cur_pos, const Vector3d& cur_vel, const Vector3d& cur_pitch, const Vector3d& cur_yaw,
    const vector<vector<Vector3d>>& n_points, const vector<vector<double>>& n_pitches, const vector<vector<double>>& n_yaws,
    vector<Vector3d>& refined_pts, vector<double>& refined_pitches, vector<double>& refined_yaws) {
  ...
  for (int i = 0; i < n_points.size(); ++i) {
    ...
    for (int j = 0; j < n_points[i].size(); ++j) {
      ViewNode::Ptr node(new ViewNode(n_points[i][j], n_pitches[i][j], n_yaws[i][j]));
  ...
  for (int i = 1; i < path.size(); ++i) {
    ...
    refined_pitches.push_back(path[i]->pitch_);
