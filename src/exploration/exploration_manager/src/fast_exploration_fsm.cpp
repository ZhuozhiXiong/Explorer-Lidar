...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  nh.param("fsm/resolution", fp_->resolution_, -1.0);
  ...
  waypoint_sub_ = nh.subscribe("/move_base_simple/goal", 1, &FastExplorationFSM::waypointCallback, this);
  ...
  state_pub_ = nh.advertise<std_msgs::Int32>("/planning/fsm_state", 10);
}
  ...
  switch (state_) {
    ...
    case PLAN_TRAJ: {
      if (fd_->static_state_) {
        ...
        fd_->start_pitch_(0) = 0.0;
        fd_->start_pitch_(1) = fd_->start_pitch_(2) = 0.0;
      } else {
...
int FastExplorationFSM::callExplorationPlanner() {
  ...
  int res = expl_manager_->planExploreMotion(fd_->start_pt_, fd_->start_vel_, fd_->start_acc_,
                                             fd_->start_pitch_, fd_->start_yaw_);
  classic_ = false;
...
void FastExplorationFSM::visualize() {
  ...
  for (int i = 0; i < ed_ptr->frontiers_.size(); ++i) {
    visualization_->drawCubes(ed_ptr->frontiers_[i], fp_->resolution_,
                              visualization_->getColor(double(i) / ed_ptr->frontiers_.size(), 1.0),
                              "frontier", i, 4);
  ...
  for (int i = ed_ptr->frontiers_.size(); i < last_ftr_num; ++i) {
    visualization_->drawCubes({Eigen::Vector3d(0, 0, 0)}, fp_->resolution_, Vector4d(0, 0, 0, 0.01), "frontier", i, 4);
  ...
  visualization_->drawSpheres(ed_ptr->refined_points_, 0.2, Vector4d(1, 0.65, 0, 1), "refined_pts", 0, 6);
  visualization_->drawLines(ed_ptr->refined_points_, ed_ptr->refined_views_, 0.05,
                            Vector4d(1, 0.65, 0, 1), "refined_view", 0, 6);
  visualization_->drawLines(ed_ptr->refined_tour_, 0.07, Vector4d(0.5, 0.5, 0, 1), "refined_tour", 0, 6);
  ...
  // Draw normals
  // visualization_->drawSpheres(ed_ptr->pts_positions_, 0.2, Vector4d(0, 0, 0.7, 0.5), "frontiers", 0, 6);
  // visualization_->drawLines(ed_ptr->pts_positions_, ed_ptr->pts_views_, 0.05, Vector4d(0, 0, 0.7, 0.5), "frontier_normals", 0, 6);
}
...
void FastExplorationFSM::frontierCallback(const ros::TimerEvent& e) {
  ...
  if (state_ == WAIT_TRIGGER || state_ == FINISH) {
    ...
    ft->searchFrontiers();
    ft->computeFrontiersToVisit(fd_->odom_pos_, fd_->odom_vel_);
    ...
    for (int i = 0; i < ed->frontiers_.size(); ++i) {
      visualization_->drawCubes(ed->frontiers_[i], fp_->resolution_,
                                visualization_->getColor(double(i) / ed->frontiers_.size(), 1.0),
                                "frontier", i, 4);
    ...
    // for (int i = 0; i < ed->dead_frontiers_.size(); ++i)
    //   visualization_->drawCubes(ed->dead_frontiers_[i], 0.1, Vector4d(0, 0, 0, 0.5), "dead_frontier",
    //                             i, 4);
    // for (int i = ed->dead_frontiers_.size(); i < 5; ++i)
    //   visualization_->drawCubes({}, 0.1, Vector4d(0, 0, 0, 0.5), "dead_frontier", i, 4);
  }
}
...
void FastExplorationFSM::waypointCallback(const geometry_msgs::PoseStampedPtr &msg) {
  if (msg->pose.position.z < -0.1) return;
  if (state_ != WAIT_TRIGGER) return;
  fd_->trigger_ = true;
  cout << "Triggered!" << endl;
  transitState(PLAN_TRAJ, "waypointCallback");
}
...
void FastExplorationFSM::odometryCallback(const nav_msgs::OdometryConstPtr& msg) {
  ...
  fd_->odom_pitch_ = asin(rot_x(2) / (rot_x.norm() + 1e-3));
