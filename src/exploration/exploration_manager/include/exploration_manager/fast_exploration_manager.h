...
public:
  ...
  int planExploreMotion(const Vector3d& pos, const Vector3d& vel, const Vector3d& acc,
                        const Vector3d& pitch, const Vector3d& yaw);
...
private:
  ...
  void findGlobalTour(const Vector3d& cur_pos, const Vector3d& cur_vel, const Vector3d cur_pitch, const Vector3d cur_yaw,
                      vector<int>& indices);

  // Refine local tour for next few frontiers, using more diverse viewpoints
  void refineLocalTour(const Vector3d& cur_pos, const Vector3d& cur_vel, const Vector3d& cur_pitch, const Vector3d& cur_yaw,
  ...
  std::string save_dir, file_name, time_log_file_;
  bool log_file_initialized_, is_logging_initialized_ = false;
