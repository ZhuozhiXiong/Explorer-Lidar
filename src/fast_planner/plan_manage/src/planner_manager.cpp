...
void FastPlannerManager::planYawExplore(const Eigen::Vector3d& start_yaw, const double& end_yaw,
  ...
  std::cout << "dt_yaw: " << dt_yaw << ", start yaw: " << start_yaw3d.transpose() * 180.0 / M_PI
  ...
  if (fabs(start_yaw3d[0] - end_yaw3d[0]) >= M_PI) {
    ...
    std::cout << "start yaw: " << start_yaw3d[0] * 180.0 / M_PI << ", " << end_yaw3d[0] << std::endl;
  }
...
void FastPlannerManager::planPitchExplore(const Eigen::Vector3d& start_pitch, const double& end_pitch,
    bool lookfwd, const double& relax_time) {
  const int seg_num = 12;
  double dt_pitch = local_data_.duration_ / seg_num;  // time of B-spline segment
  Eigen::Vector3d start_pitch3d = start_pitch;
  std::cout << "dt_pitch: " << dt_pitch << ", start pitch: " << start_pitch3d.transpose() * 180.0 / M_PI
            << ", end: " << end_pitch << std::endl;
  
  double last_pitch = start_pitch3d[0];

  // Pitch traj control points
  Eigen::MatrixXd pitch(seg_num, 1);
  pitch.setZero();

  // Initial state
  // Eigen::Matrix3d states2pts;
  // states2pts << 1.0, -dt_pitch, (1 / 3.0) * dt_pitch * dt_pitch, 1.0, 0.0, -(1 / 6.0) * dt_pitch * dt_pitch,
  //     1.0, dt_pitch, (1 / 3.0) * dt_pitch * dt_pitch;
  // pitch.block<3, 1>(0, 0) = states2pts * start_pitch3d;

  vector<Eigen::Vector3d> waypts;
  vector<int> waypt_idx;

  // Final state
  Eigen::Vector3d end_pitch3d(end_pitch, 0, 0);
  // pitch.block<3, 1>(seg_num, 0) = states2pts * end_pitch3d;

  // Debug rapid change of pitch
  if (fabs(start_pitch3d[0] - end_pitch3d[0]) >= M_PI) {
    ROS_ERROR("Pitch change rapidly!");
    std::cout << "start pitch: " << start_pitch3d[0] * 180.0 / M_PI << ", " << end_pitch3d[0] << std::endl;
  }

  // Interpolate start and end value for smoothness
  for (int i = 0; i < seg_num; ++i)
  {
    Eigen::Vector3d waypt = (1 - double(i) / seg_num) * start_pitch3d + double(i) / seg_num * end_pitch3d;
    waypts.push_back(waypt);
    waypt_idx.push_back(i);
    pitch(i, 0) = waypt(0);
    last_pitch = waypt(0);
  ...
  // int cost_func = BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::START | BsplineOptimizer::END |
  //                 BsplineOptimizer::WAYPOINTS;
  // vector<Eigen::Vector3d> start = { Eigen::Vector3d(start_pitch3d[0], 0, 0),
  //   Eigen::Vector3d(start_pitch3d[1], 0, 0), Eigen::Vector3d(start_pitch3d[2], 0, 0) };
  // vector<Eigen::Vector3d> end = { Eigen::Vector3d(end_pitch3d[0], 0, 0), Eigen::Vector3d(0, 0, 0) };
  // bspline_optimizers_[1]->setBoundaryStates(start, end);
  // bspline_optimizers_[1]->setWaypoints(waypts, waypt_idx);
  // bspline_optimizers_[1]->optimize(pitch, dt_pitch, cost_func, 1, 1);

  // std::cout << "2: " << (ros::Time::now() - t1).toSec() << std::endl;

  // Update traj info
  local_data_.pitch_traj_.setUniformBspline(pitch, 3, dt_pitch);
  local_data_.pitchdot_traj_ = local_data_.yaw_traj_.getDerivative();
  local_data_.pitchdotdot_traj_ = local_data_.yawdot_traj_.getDerivative();
  plan_data_.dt_pitch_ = dt_pitch;
