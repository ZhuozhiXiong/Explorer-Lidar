...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  nh.param("sdf_map/truncate_height", mp_->truncate_height_, -0.1);
  nh.param("sdf_map/truncate_low", mp_->truncate_low_, -0.1);
  ...
  md_->distance_buffer_obs_ = vector<double>(buffer_size, mp_->default_dist_);
  ...
  for (int x = min_id(0); x <= max_id(0); ++x)
    for (int y = min_id(1); y <= max_id(1); ++y)
      for (int z = min_id(2); z <= max_id(2); ++z) {
        ...
        md_->distance_buffer_obs_[toAddress(x, y, z)] = 100.0;
      }
}
...
void SDFMap::updateESDF3d() {
  ...
  // --- 2. [NEW] Obstacle-Only ESDF Update ---
  // We reuse tmp_buffer1_ and tmp_buffer2_ since the first update is finished.
  for (int x = min_esdf[0]; x <= max_esdf[0]; x++)
    for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
      fillESDF(
          [&](int z) {
            // Check if voxel is occupied
            bool is_occ = md_->occupancy_buffer_[toAddress(x, y, z)] > mp_->min_occupancy_log_;
            
            // Calculate real height of this voxel
            double z_height = (z + 0.5) * mp_->resolution_ + mp_->map_origin_(2);
            
            // Treat as obstacle ONLY if it is occupied AND above the ground threshold
            if (is_occ && z_height > mp_->truncate_low_) {
               return 0.0;
            } else {
  ...
  for (int y = min_esdf[1]; y <= max_esdf[1]; y++)
    for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
      fillESDF(
          [&](int x) { return md_->tmp_buffer2_[toAddress(x, y, z)]; },
          [&](int x, double val) {
            // Save result to the new buffer
            md_->distance_buffer_obs_[toAddress(x, y, z)] = mp_->resolution_ * std::sqrt(val);
          },
          min_esdf[0], max_esdf[0], 0);
    }
}
...
void SDFMap::inputPointCloud(
  ...
  for (int i = 0; i < point_num; ++i) {
    ...
    if (!isInMap(pt_w)) {
      ...
      if (length > mp_->max_ray_length_ + mp_->resolution_ * 0)
        pt_w = (pt_w - camera_pos) / length * mp_->max_ray_length_ + camera_pos;
      if (pt_w[2] < mp_->truncate_low_ || pt_w[2] > mp_->truncate_height_) continue;
      tmp_flag = 0;
    } else {
      length = (pt_w - camera_pos).norm();
      if (length > mp_->max_ray_length_ + mp_->resolution_ * 0) {
  ...
  while (!md_->cache_voxel_.empty()) {
    ...
    // double log_odds_update =
    //     md_->count_hit_[adr] >= 1 ? mp_->prob_hit_log_ : mp_->prob_miss_log_;
...
void SDFMap::kdtreeBoxSearch(const BoxPointType& box, PointVector& pts) {
  md_->kdtree_->Box_Search(box, pts);
}
