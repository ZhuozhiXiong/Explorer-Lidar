...
public:
  ...
  double getDistToObstacle(const Eigen::Vector3d& pos);
  ...
  void setCover(const Eigen::Vector3i& idx);

  void kdtreeBoxSearch(const BoxPointType& box, PointVector& pts);
...
struct MapParam {
  ...
  double truncate_height_, truncate_low_;
};
...
struct MapData {
  ...
  std::vector<double> distance_buffer_obs_;
...
inline void SDFMap::setCover(const Eigen::Vector3i& idx)
{
  if (!isInBox(idx))
    return;
  md_->cover_occ_[toAddress(idx)] = 1;
}
...
inline double SDFMap::getDistToObstacle(const Eigen::Vector3d& pos) {
  Eigen::Vector3i idx;
  posToIndex(pos, idx);
  if (!isInBox(idx)) return 100.0; // Outside map considered far from obstacles
  return md_->distance_buffer_obs_[toAddress(idx)];
}
