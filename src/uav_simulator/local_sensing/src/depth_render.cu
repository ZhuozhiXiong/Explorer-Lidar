...
//	int r = 0.0473 * para.fx / dist + 0.5f;
	int r = para.render_pt_size * para.fx / dist + 0.5f;
	for(int i = -r; i <= r; i++)
		for(int j = -r; j <= r; j++)
		{
...
void DepthRender::set_para(float _fx, float _fy, float _cx, float _cy, int _width, int _height, double _render_pt_size)
{
	parameter.fx = _fx;
	parameter.fy = _fy;
	parameter.cx = _cx;
	...
	parameter.render_pt_size = _render_pt_size;
}
